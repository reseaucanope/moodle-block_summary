...
class block_summary_renderer extends plugin_renderer_base {
    ...
    private function render_node($node, $courseid, $canseehiddensections, $classes)
    {
        ...
        if(empty($node->children)){
            ...
            if (section_is_completed($node->sectionid))
            {
                $completed = '<a class="summary-modular-nomodule-completed" title="Vous avez achevé cette étape en cochant la case sur la page correspondante" style="padding-left: 0;position: absolute;left: -16px;"><i class="fa fa-check"></i></a> ';
            }
            $ret = $completed.'<span class="arrow fa fa-caret-right"></span>'.$namehtml;
            if(isset($node->current)){
                $ret = '<b>'.$ret.'</b>';
            }
            return '<li'.($completed!=''?' style=""':'').'>'.$ret.'</li>';
        }
        ...
        return '<li'.($completed!=''?' style=""':'').'>'.$html.'</li>';
    }
